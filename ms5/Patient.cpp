...
Time Patient::time() const { return m_ticket.time(); }

// Establish a query method named number that returns the number associated with
// the patient's ticket.
int Patient::number() const { return m_ticket.number(); }

Patient::operator bool() const {
  if (m_PatientName != nullptr && m_ohipNum >= 100000000 &&
      m_ohipNum <= 999999999) {
    return true;
  }
  return false;
...
Patient::operator const char *() const { return m_PatientName; }
