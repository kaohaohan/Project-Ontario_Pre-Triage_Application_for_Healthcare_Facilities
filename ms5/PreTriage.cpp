...
namespace seneca{
        ...
        PreTriage::PreTriage(const char *dataName):m_waitTimeForContaigenTest(15),m_waitTimeForTriagePatients(5){
            if (dataName!=nullptr){
        ...
        void PreTriage::setAverageWaitTime(Patient& patient){
            ...
            int result = ((currentTime-patientTicketTime)+(averageWaitTime *(patientTicketNum-1)))/patientTicketNum;
            //不確定這樣寫 有錯可以看這
            if(patient.type()=='T'){
        ...
        void PreTriage::load(){
           ...
           int count =0;
            ...
            if (file.peek() != EOF) {
                  cout << "Warning: number of records exceeded " << MAX_PATIENT_LENGTH << endl;
        ...
        void PreTriage::save()const {
            ...
            file << m_waitTimeForContaigenTest << ','<< m_waitTimeForTriagePatients<< endl;
            for (int i=0; i<m_numPatientLineup;i++){
